...
import { isAnyArrayBuffer, isDate, isRegExp } from './utils.ts';
...
function calculateElement(
  ...
  switch (typeof value) {
    case 'string':
      return 1 + ByteUtils.utf8ByteLength(name) + 1 + 4 +
        ByteUtils.utf8ByteLength(value) + 1;
    case 'number':
      if (
        ...
        if (
          value >= constants.BSON_INT32_MIN && value <= constants.BSON_INT32_MAX
        ) {
          // 32 bit
    ...
    case 'object':
      if (
        value == null || value['_bsontype'] === 'MinKey' ||
